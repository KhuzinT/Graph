...
class GraphAlgorithm {
    ...
    friend std::set<Vertex> FindArticulationPoints(Graph& G, const uint64_t& q_vertex);

};

std::set<Vertex> FindArticulationPoints(Graph& G, const uint64_t& q_vertex) {
    ...
    for (auto &element : GAlg.component) {
        if (element >= q_vertex) {
            answer.insert(element - q_vertex + 1);
        }
    }
    return answer;
}
...
int main() {
    ...
    GraphList G(q_vertex + q_edge, NOT_ORIENTED);

    Vertex not_used_vertex = q_vertex + 1;
    for (uint64_t i = 0; i < q_edge; ++i) {
        Vertex a = 0, b = 0, c = 0;
        std::cin >> a >> b >> c;
        G.AddEdge(a, not_used_vertex, 0);
        G.AddEdge(b, not_used_vertex, 0);
        G.AddEdge(c, not_used_vertex, 0);
        ++not_used_vertex;
    }

    auto answer = FindArticulationPoints(G, q_vertex);
