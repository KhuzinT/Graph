...
public:
    ...
    [[nodiscard]] bool IsNotMultiEdge(const Vertex& v, const uint64_t index) const {
        return list[v][index].quantity_edge == 1;
    }

    [[nodiscard]] uint64_t GetNumber(const Vertex& v, const uint64_t& index) const {
...
class GraphAlgorithm {
    ...
    friend std::set<uint64_t> FindBridges(GraphList& G);
};

std::set<uint64_t> FindBridges(GraphList& G) {
