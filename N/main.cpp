...
class GraphAlgorithm {
    ...
    void Sort(Graph& G, const Vertex& v) {
        color[v] = GREY;
        auto neighbors = G.GetNeighborsVertex(v);
        for (auto &u : neighbors) {
            if (color[u] == WHITE) {
    ...
    friend std::vector<Vertex> TopologicalSort(Graph& G);
};
...
std::vector<Vertex> TopologicalSort(Graph& G) {
    if (IsCycle(G)) {
        return {};
    }
    ...
    std::vector<Vertex> answer;
    while (!GAlg.component.empty()) {
        answer.push_back(GAlg.component.top() + 1);
        GAlg.component.pop();
    }
    return answer;
}
